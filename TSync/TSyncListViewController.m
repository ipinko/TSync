...
{
    NSLog(@"loadMore");
    sinceID = [_engine getFollowedTimelineSinceID:sinceID startingAtPage:0 count:5];
}

-(void)loadingMore:(UITableViewCell*)cell
{
    NSLog(@"loadingMore");
    
    cell.textLabel.text=@"loading more …";
    UIImage* spacer = [UIImage imageNamed:@"spacer"];
    CGSize size= activityIndicator.frame.size;
    UIGraphicsBeginImageContext(size);
    [spacer drawInRect:CGRectMake(0, 0, size.width, size.height)];
...
- (void)viewDidAppear: (BOOL)animated {
    ...
    NSLog(@"viewDidAppear again,engine:%@",_engine);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([list count]<1 && !more){
        [self loadingMore:cell];
    }
